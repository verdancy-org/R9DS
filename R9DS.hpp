...
class R9DS : public LibXR::Application {
 ...
 private:
  ...
  void UpdateRcState() {
    ...
    rc_state_.throttle_low =
        rc_state_.channels[CH_THR] < -CALI_STICK_TRIGGER_DEF;
    ...
    rc_state_.throttle = Clamp(
        static_cast<float>(rc_state_.channels[CH_THR] + 500) / 1000.0f, 0.0f,
        1.0f);
